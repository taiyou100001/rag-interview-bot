    ...
    def _gemini_score_resume(self, resume_text: str) -> dict:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                prompt = (
                    "請以專業人資角度，針對以下履歷內容給一個 0~100 分的分數，忽略排版與結構，只針對內容與他所應徵的職位進行綜合評分，並簡要說明約一百個字的理由：\n"
                    f"{resume_text}\n"
...
class FileManager:
    @staticmethod
    def save_resume_score(score_result: dict, filename: str = None) -> str:
        """
        將履歷評分與評語（score_result）儲存為獨立 JSON 檔。
        預設檔名為 resume_score_{日期時間}.json。
        """
        import datetime
        if not filename:
            now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"resume_score_{now}.json"
        # 只保留分數與評語（若有）
        output = {}
        ...
        if "reason" in score_result:
            output["reason"] = score_result["reason"]
        if "gemini_score" in score_result and isinstance(score_result["gemini_score"], dict):
            output["gemini_score"] = score_result["gemini_score"]
        if "original_file_score" in score_result and isinstance(score_result["original_file_score"], dict):
            output["original_file_score"] = score_result["original_file_score"]
